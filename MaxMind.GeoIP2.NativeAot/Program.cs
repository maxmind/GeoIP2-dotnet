...
if (RuntimeFeature.IsDynamicCodeSupported || System.Text.Json.JsonSerializer.IsReflectionEnabledByDefault)
{
    throw new InvalidOperationException(
        "Dynamic code and default JSON reflection must be disabled. Run dev-bin/test-native-aot.sh with a runtime identifier.");
}
...
{
    throw new InvalidOperationException("The database lookup failed.");
}
...
{
    throw new InvalidOperationException("JSON decoding or locale copying failed.");
}
Console.WriteLine(".NET 8 smoke test passed.");
